    ...
    do
        if echo "$iface" | grep "^wl"; then
            ...
            if echo "$config" | grep -qv "inet "; then              # Test if not currently connected
                ...
                info="Wlan will be selected"                            # Get the iface hardware infos.
                if echo "$info" | grep "Wlan"; then            # Know if the iface has the AP mode available.
